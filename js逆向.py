...
function getsu(user){
su = sinaSSOEncoder.base64.encode(encodeURIComponent(user));
return su}
;
'''
...
function getsp(pwd,me){
var f = new sinaSSOEncoder.RSAKey;
f.setPublic(me.pubkey, "10001");
sp = f.encrypt([me.servertime, me.nonce].join("\\t") + "\\n" + pwd);
return sp}
;'''
...
def get_su_sp(pwd,me):
    results_su=execjs.compile(getsu).call('getsu',user)
    #rsaPubkey修改为pubkey
    results_sp=execjs.compile(getsp).call('getsp',pwd,me)
    print(results_su,results_sp)
    return results_su,results_sp
...
def get_me():
    ...
    headers={
        ...
        'Cookie':'SUB=_2AkMpOD2gf8NxqwJRmf0UxG_la4VyzgHEieKfZMx7JRMyHRl-yD92qkoNtRB6ArgTT6F_65FWMIEFMrrbStx8kTJ9bl-m; SUBP=0033WrSXqPxfM72-Ws9jqgMF55529P9D9W5LA49yR96Zu_mxUua5uwf3; login=07d0fc3f1785c8c156912874508f4cfb; SINAGLOBAL=61.144.173.230_1583657626.474759; Apache=61.144.173.230_1583657626.474760',
    }
    response=session.get(pre_url,params=params,headers=headers).text
    #eval() 函数用来执行一个字符串表达式，并返回表达式的值。
    response=eval(response.replace('sinaSSOController.preloginCallBack',''))
    print(response)
    return response

def get_location(response):
    pattern=re.compile('(https.*?)"\)')
    location_url=re.findall(pattern,response)
    print(location_url)
    return location_url[0]
...
def login(me,su,sp):
    ...
    headers={
        ...
        'Connection':'keep-alive',
        'Referer':'https://weibo.com/',
        'Cookie':'SUB=_2AkMpODQ3dcPxrABXmP8TzmzkaYlH-jya7V3BAn7tJhMyAhhu7l0OqSVutBF-XNCgpzDmAUDKIapcic5oQIbNQHI6; SUBP=0033WrSXqPxfM72wWs9jqgMF55529P9D9WhJmF9cZLSA8s954vVJZxn35JpVF02f1KnEe02N1KB7; login=07d0fc3f1785c8c156912874508f4cfb; SINAGLOBAL=61.144.173.230_1583657626.474759; Apache=61.144.173.230_1583657626.474760; SCF=AnhVCMl-Tc3c_1fZFtYCZkMrLjh-EZb9CR7671GrAgPWf2cKrsaZx2FwMrvqWok2Eg2jbodEezncAaoC__To2Ck.',
        'Upgrade-Insecure-Requests':'1',
    }
    response=session.post(url,data=data,headers=headers)
    response=response.content.decode('gbk')
    location_url=get_location(response)
    return location_url
...
def location(location_url):
    ...
    response=session.get(location_url,headers=headers).content.decode('gbk')
    save('location_url.html',response)
    return response

def location_parse(response):
    '''
    解码后：https://passport.weibo.com/wbsso/login?ticket=ST-MzYwNzYyNzY1NA%3D%3D-1583669982-gz-B7ED22325F494995A75A6381B5E58CC2-1&ssosavestate=1615205980&callback=sinaSSOController.doCrossDomainCallBack&scriptId=ssoscript0&client=ssologin.js(v1.4.19)&_=1583669982330
    解码前：https://passport.weibo.com/wbsso/login?ticket=ST-MzYwNzYyNzY1NA%253D%253D-1583672792-gz-325360F7BE81B3A2914191DCC9CB3F0E-1&ssosavestate=1615208792&callback=sinaSSOController.doCrossDomainCallBack&scriptId=ssoscript0&client=ssologin.js%28v1.4.19%29&_=1583672792499
    如果不解码，url会不一样
    '''
    response=parse.unquote(response)
    pattern=re.compile('ssosavestate=(.*?)&url')
    ssosavestate=re.findall(pattern,response)
    print(ssosavestate[0])
    pattern=re.compile('ticket=(.*?)&ssosavestate')
    ticket=re.findall(pattern,response)
    print(ticket[0])
    return ssosavestate[0],ticket[0]

def ticket_login(ssosavestate,ticket):
    ...
    params={
        'ticket':ticket,#存在于location网页中
        'ssosavestate':ssosavestate,#存在于location网页中
    ...
    print(response.url)
    save('ticket.html',response.content.decode('gbk'))

def my_login(my_url):
    headers={
        'User-Agent': 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:73.0) Gecko/20100101 Firefox/73.0',
    }
    response=session.get(my_url,headers=headers).text
    save('my_url.html',response)


def main():
    #第一步：登陆前期准备，把账号密码传入js逆向函数，获得登陆时需要的参数me,su,sp
    me=get_me()
    su,sp=get_su_sp(pwd,me)
